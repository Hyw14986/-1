...
1. 仅用户手动点击「开始寻找」触发完整查询流程，且整套查询全部成功后才消耗 1 次查询次数、缓存腾讯 POI、新增一条查询记录；查询任一步失败（云函数异常 / 接口报错 / 网络异常）不扣次数、不生成查询记录；筛选、查看详情、再次查询回填半径都不消耗次数
...
8. 查询结果分支：①自有库有数据 + 腾讯接口失败 → 仅展示本地库数据，toast「地图接口请求异常，仅展示本地库数据」；②自有库为空 + 腾讯有数据 → 正常渲染腾讯过滤后 POI；③两边都无数据 → 空状态弹窗（扩大查询半径 / 上报厕所）；④接口异常 → 不扣次数并 toast「查询失败，本次未消耗次数，请稍后重试」。腾讯 place 返回为空不判定整体失败，优先使用自有数据库数据

...
基于微信云开发（云数据库 + 云存储 + 云函数）的找厕所小程序：按半径查找周边公厕、评分评价、上报新点位、查询次数配额管理。

## 功能一览

- **首页（找厕所不迷路）**：页面标题「找厕所不迷路」，左上角下拉选择查询半径（500 / 1000 / 2000 / 3000 米），点击「开始寻找」才执行查询，整套查询成功后才消耗 1 次查询次数（每日上限 20 次，0 点自动重置，失败不扣次数）；地图中心永久锁定用户 GCJ-02 定位，绘制红色圆形查询圈，只渲染圈内公厕 marker；底部卡片统计「在您附近找到 X 个厕所」并可展开按距离排序的列表；点击 marker 打开详情弹窗（来源标签、设施标签、平均分、评价列表、一键前往 / 收藏 / 写评价 / 举报）；顶部「🗺️ 一键显示全部厕所」按钮读取 toiletAll 全部有效点位（按距用户由近到远排序、include-points 视野覆盖、不消耗查询次数），并实时展示「🗄️ 数据库已收录 X 座公厕」总量
...
- **公厕排行榜页**：聚合网络公开报道的网红/最美公厕（`toilet_hot` 种子数据）与用户真实评分（`toilet_comment` 聚合），提供「全网人气榜 / 用户口碑榜」双榜单，领奖台 Top3 + 奖牌列表，点击可查看上榜理由、网络评价摘录并一键前往
- **查询记录页**：展示每次点击「开始寻找」的历史记录（查询时间、半径、找到厕所数量）；支持「再次查询」（回填半径回首页，不扣次数，需再点开始寻找）、单条删除、一键清空
- **洗手计时器**：纯本地 20 秒科学洗手工具，阶段提示、环形进度、完成震动，不联网、不写库、无 UGC
- **视觉主题**：渐变 Hero + 玻璃拟态卡片 + 微动效；动效关键帧使用 `wx-wow` 裁剪版（`utils/motion.wxss`），未整包引入重型 UI 库

## UI 组件（Vant Weapp 按需接入）

为避免整包引入导致主包超过 2MB，本项目按需拷贝了有赞 `youzan/vant-weapp`（MIT 协议，dev 分支）的三个轻量组件，直接维护在 `miniprogram_npm/@vant/weapp/`（本机 npm 执行策略受限，改为手工拷贝并入库）：

| 组件 | 用途 | 使用页面 |
| --- | --- | --- |
| `van-skeleton` | 骨架屏占位，优化查询 / 列表加载等待 | 首页底部托盘（查询中）、厕所列表页（加载中） |
| `van-loading` | 加载动画（spinner 类型） | 首页地图浮层、列表页、排行榜两页 |
| `van-empty` | 空状态容器（保留项目 SVG 图标与文案，slot 定制） | 列表页、查询记录页、排行榜两页 |

组件已在 `app.json` 全局注册（`usingComponents`），无需逐页声明；样式通过 `custom-class` 与各页 wxss 适配当前「苹果白简约」主题。若需升级组件：从 https://github.com/youzan/vant-weapp 下载对应分支，仅覆盖 `lib/` 下的 `skeleton`、`loading`、`empty`、`common`、`mixins`、`wxs`、`definitions` 目录即可。

> 注意：`.gitignore` 已为 `miniprogram_npm/@vant/weapp/` 开放例外，组件必须随代码一起提交，否则全新克隆后首页 / 列表 / 空状态组件会缺失。
...
### 7. 配置腾讯位置服务 Key（可选增强）
?? `config/keys.example.js` ? `config/keys.js`?? `QQ_MAP_KEY` ???????????? WebServiceAPI Key?
...
- **??**?? `config/keys.js` ? `AMAP_KEY` ???????Web???Key??????? [??????](https://console.amap.com/) ???request ??????? `https://restapi.amap.com`?????????????? QPS 3????????????`AMAP_MAX_CONCURRENCY`??? 3???????????????? infocode=10045?QPS ???
...
- **???**?Key ????????????? `cloudfunctions/searchTiandituPoi/key.example.js` ? `key.js` ??? `TIANDITU_KEY`????????????????????????????? Key ?? 403?code 301013 ??????????????????????????????? `searchTiandituPoi` ??????????????????
...
- **已内置可导入**（有经纬度）：`importCityToilets`（2026-08-08 扩充至 **38,465 条**：北京 8401 + 上海 4081 + 武汉 3131 + 广州 2842 + 深圳 3578 + 成都 3008 + 杭州 3676 + 重庆 2537 + 西安 2626 + 南京 2082 + 郑州 2016 + 湛江 487；来源：高德 place/around 网格采集 34,685（types=200300，半径 2000m 格距 2800m）+ 连锁品牌洗手间 3,436（肯德基/麦当劳/星巴克/瑞幸咖啡，source='chain'）+ 腾讯 POI 199 + OSM 145）、`importGovToilets`（达州宣汉旅游厕所 45 条 + 宿迁洋河新区公厕 34 条，CGCS2000≈WGS-84，入库自动转 GCJ-02）。部署后在云开发控制台右键对应函数 → 云端测试即可，幂等可重复执行；如云函数超时，可传参数 `{"start":0,"limit":8000}` 分片续导（再次运行同一函数补齐剩余区间即可）
- **批量采集工具**：`tools/collectCityToilets.js`（高德网格遍历 + 连锁品牌洗手间，并发 3 防 10045、断点续采、连续失败熔断；百度源需在工具内配置 Referer/SN，腾讯源配额紧张默认不参与批量采集），运行 `node tools/collectCityToilets.js --all` 生成 `tools/output/<城市>.json`，再用 `node tools/mergeIntoData.js` 去重合并进 `importCityToilets/data.js`
- **调研结论（未内置）**：北海市 473 条（`bh.data.gxzf.gov.cn` 可直接下载但无经纬度，纯文本地址需地理编码，成本高未纳入）；深圳/广州/山东平台存在公厕数据集，但下载需登录或字段不含坐标，待后续单独对接
- **住建部「城市公厕云平台」调研结论（不可接入）**：原入口 `lavatory.cnues.com` 域名已过期进入出售状态（实测为 4.cn 出售页），平台已停止运营，且从未公开过 API / 数据接口，无法接入
- 政府数据统一写入 `toiletAll`，`source='gov'`、`auditStatus='pass'`、含 `loc` 地理字段，可直接参与 geoNear 圈内查询与地图渲染

### 7.7 政务开放数据对接指引（方案 A/D，注册 appKey 后可执行）
已实测各省市开放平台：**除内蒙古外全部需要注册账号申请 appKey 或登录后下载**；内蒙古旅游厕所 966 条可匿名读取但无经纬度（仅名称），未纳入。以下数据集字段含经纬度、最值得对接，注册后按平台文档申请密钥即可批量导入（建议复用 `importGovToilets` 的导入模式）：
- **深圳**（`opendata.sz.gov.cn`）：龙华区城管局公共厕所信息（接口 `api/29200_04703877/1/service.xhtml`，约 60 座，需 appKey）；龙岗区公共厕所信息（CSV/JSON/XLS/XML 四格式，字段含经纬度）；宝安区公共厕所数据（XLSX）
- **无锡**（`data.wuxi.gov.cn`）：环卫公厕信息 2023 分页查询服务（接口 `devweb/api/cataLog_87f98aad43b249378b91f85cfb35ee23/get_datalist_page/1.0`）、滨湖区公厕一览表、宜兴城区公厕，均需 appKey（匿名访问 401 credentialsNotFound）
- **温州**（`data.wenzhou.gov.cn`）：鹿城区智融瞭望系统-公厕信息（iid=6680，字段含**高德经度/高德纬度**，GCJ-02 可直接入库）；温州城市大脑文旅系统-旅游厕所（iid=9653，天地图经纬度）；龙湾区公共厕所地理信息（iid=16465，约 339 条）；下载接口 `jdop_front/download.do?fileType=csv&iid=<id>` 需登录会话
- **十堰**（`opendata.shiyan.gov.cn`）：公共厕所分布点位信息查询接口 `gateway/api/1/queryggcs`（有条件开放，需申请权限）
- **宜昌**（`data.yichang.gov.cn`）：城区环卫公共厕所信息（表 1001390）、伍家岗区公厕信息表（表 1002173），前端 SPA 数据接口在 `/athena` 下，需登录后调用
- **荆州**（`data.jingzhou.gov.cn:10443`）：城市公厕信息数据共享（xls 194KB，字段含公厕名称/位置，无经纬度需地理编码；站点证书已过期，访问需忽略证书）
- **广州**：广州市公共厕所信息（行政区/街道/名称/地址/等级/蹲位数/残座数/开放时间，无经纬度，需地理编码）
- 对接流程建议：注册平台账号 → 申请数据接口 appKey → 脚本拉取 CSV/JSON → 有经纬度直接转 GCJ-02 入库；无经纬度用高德地理编码（`v3/geocode/geo`，注意配额）补坐标 → 合并进 `importGovToilets/data.js`（`source='gov'`）→ 云端测试导入
...
│   ├── list/                        # （旧版列表页，暂未引用）
│   └── detail/                      # （旧版详情页，暂未引用）
├── tools/                           # 批量采集工具（collectCityToilets 网格采集 / mergeIntoData 合并导入）
